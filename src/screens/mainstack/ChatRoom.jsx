...
const ChatRoom = () => {
  ...
  return (
    <div>
        ...
        {messages?.map((item, index) => (
          ...
          >
            <div
              className={
                item?.senderId === location?.state?.senderId
                  ? "flex w-fit justify-end bg-[#DCDCDC] px-[10px] py-[5px] max-w-[60%] rounded-md"
                  : "flex w-fit justify-start bg-[#87CEFA] px-[10px] py-[5px] max-w-[60%] rounded-md"
              }
            >
      ...
      <div
        className="flex gap-3 flex-row justify-center items-center py-[10px] px-[10px] bg-slate-300"
        style={{
          margin: "auto",
        }}
      >
        ...
        <input
          className="w-[80%] h-[40px] rounded-full bg-white p-[10px] focus:outline-none"
          value={message}
          onChange={(e) => setMessage(e.target.value)}
          placeholder="Type your message..."
        />
        <button
          className="bg-[] text-white rounded-full p-[2px] h-[40px] w-[8%] focus:outline-none"
          onClick={() =>
            sendMessage(location?.state?.senderId, location?.state?.receiverId)
          }
        >
          <IoMdSend size={24} color="#1E90FF" style={{ margin: "0" }} />
        </button>
      </div>
    </div>
  );
